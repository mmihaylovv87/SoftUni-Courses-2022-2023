...
namespace _08.Bombs
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            string[] indexPairs = Console.ReadLine().Split(" ", StringSplitOptions.RemoveEmptyEntries);
            foreach (var indexPair in indexPairs)
            {
                int[] indices = indexPair.Split(",", StringSplitOptions.RemoveEmptyEntries).Select(i => int.Parse(i)).ToArray();
                int row = indices[0];
                int col = indices[1];
