...
class Program
{
    ...
    static bool TeamExists(List<Team> teams, string teamName) => teams.Any(t => t.Name == teamName);

    static bool AlreadyCreatedATeam(List<Team> teams, string creator) => teams.Any(t => t.Creator == creator);
    ...
    static bool AlreadyAMemberOfATeam(List<Team> teams, string user) =>
        teams.Any(t => t.Members.Contains(user)) || teams.Any(t => t.Creator == user);
...
class Team
{
    ...
    public void AddMember(string memberName) => this.members.Add(memberName);
}